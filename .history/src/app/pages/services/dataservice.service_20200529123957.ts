 ...
 /*  addDeveloper(name, skills, img) {
  ...
  getSongs(id): Promise<Song> {
    return this.database.executeSql('SELECT * FROM song WHERE id = ?', [id]).then(data => {
      let songs = [];
      if (data.rows.item(0).songs != '') {
        songs = JSON.parse(data.rows.item(0).songs);
      }
      ...
      return {
        ...
        verse2: data.rows.item(0).verse1,
        verse3: data.rows.item(0).verse1,
        verse4: data.rows.item(0).verse1,
        verse5: data.rows.item(0).verse1,
        verse6: data.rows.item(0).verse1,
      }
    });
  }
